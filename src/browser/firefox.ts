...
const getNativeMessageManifest = (binPath: string): NativeMessageManifest => {
  const manifestGeckoID = (addonManifest as any)?.applications?.gecko?.id;
...
const getNativeMessageManifestJson = (binPath: string): string => {
  return JSON.stringify(getNativeMessageManifest(binPath), null, 2);
};
